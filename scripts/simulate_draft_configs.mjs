...
//
// Usage: node scripts/simulate_draft_configs.mjs
...
function loadPlayers() {
  const rows = loadCsv(path.join(ROOT, "player_data.csv"));
...
function main() {
  ...
  const configs = loadCsv(path.join(ROOT, "draft_config_permutations.csv"));
  const resultsPath = path.join(ROOT, "draft_config_simulation_results.csv");
  ...
  writeFileSync(path.join(ROOT, "draft_config_simulation_results.csv"), out.join("\n") + "\n", "utf8");
